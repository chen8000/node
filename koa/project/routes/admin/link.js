...
router.get('/', async (ctx) => {
    await ctx.redirect(`${ctx.state.__HOST__}/admin/link/list?page=1`);
})
...
router.get('/list', async (ctx) => {
        ...
        await ctx.render('admin/link/list', { result , page, pageSize, counts});
});
...
router.post('/doAdd', upload.single('pic'), async (ctx) => {
    ...
    ctx.redirect(`${ctx.state.__HOST__}/admin/link/list?page=1`);
});
...
router.get('/edit', async (ctx) => {
    ...
    ctx.render('admin/link/edit', {result, prevPage});
});
